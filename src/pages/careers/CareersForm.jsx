...
const CareerForm = () => {
  ...
  const { data, isLoading } =
    role === "superadmin"
      ? useGetCareersFormQuery()
      : useGetCareersFormByDepartmentQuery(department);
  const [deleteCareerForm] = useDeleteCareersFormMutation();
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [openOptionIndex, setOpenOptionIndex] = useState(null);
  const [selectedCareerForm, setSelectedCareerForm] = useState(null);
  ...
  const handleView = (afflicates) => {
    setSelectedCareerForm(afflicates);
    setIsModalOpen(true);
  };

  const handleDelete = async (id) => {
    try {
      await deleteCareerForm(id).unwrap();
      toast.success("Career form deleted successfully");
    } catch (error) {
  ...
  const handleExport = async () => {
    try {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/careers/careers-form/export?${query.toString()}`,
        {
          method: "GET",
          headers: {
            Accept:
  ...
  return (
    <>
      <div className="flex items-center justify-between gap-5">
        <Heading className="text-xl md:text-2xl">CareerForm</Heading>
            ...
            <tbody>
              {data?.data?.map((d, idx) => (
                ...
                >
                  ...
                  <td className="p-2">{d?.selectedOpening?.title?.en}</td>
      ...
      <CareersSubmissionForm
        isOpen={isModalOpen}
        onClose={() => {
          setSelectedCareerForm(null);
          setIsModalOpen(false);
        }}
        careerForm={selectedCareerForm}
      />
    </>
  );
};
